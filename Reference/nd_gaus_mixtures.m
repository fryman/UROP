function [bestk,bestpp,bestmu,bestcov,dl,countf] = nd_gaus_mixtures(y,w,kmin,kmax,regularize,th,covoption)
%
%
% Usage syntax:
...
% covoption = 0 means free covariances - default
...
%
%
%     This is a straight up implementation of the algorithm described in the
%     paper "Unsupervised Learning of FInite Mixture Models" Figueiredo and
%     Jain.  IEEE transactions on pattern analysis and machine intelligence
%     Vol 24 No 3.
%     Original 1D and 2D code written by
%   Mario Figueiredoy
...
for i=1:k
    ...
    estcov(:,:,i) = globcov*0.1;
end


% we plot the data and the initial estimates
if (dimens == 1)
    [hh,xx] = hist(y,bins);indic(1:k,:)
    ...
    for comp=1:k
        elipsnorm(estmu([axis1,axis2],comp),...
            estcov([axis1,axis2],[axis1,axis2],comp),[.5,1,2,3])
    end
    drawnow
    hold off
end
...
%keyboard
for i=1:k
    %indic varianbles are indicator functions
    %semi_indic(i,:) = mvnpdf(y',estmu(:,i)',estcov(:,:,i));
...
countf = 1;
%size(repmat(W,[size(indic,1),1]))
%size(indic)
loglike(countf) = sum(W.*log(sum(realmin+indic)));  %%DBUG--WEIGHTS
dlength = -loglike(countf) + (nparsover2*sum(log(estpp))) + ...
    (nparsover2 + 0.5)*k*log(sum(W));  %%DBUG--WEIGHTS
dl(countf) = dlength;
kappas(countf) = k;
...
while(k_cont)  % the outer loop will take us down from kmax to kmin components
    ...
    while(cont)    % this inner loop is the component-wise EM algorithm with the
        ...
        while comp <= k
            ...
            normindic = indic./(realmin+kron(ones(k,1),sum(indic,1))).*repmat(W,[size(indic,1),1]);
            
            % now we perform the standard M-step for mean and covariance
            normalize = 1/sum(normindic(comp,:));
            aux = kron(normindic(comp,:),ones(dimens,1)).*y;  %%DBUG--WEIGHTS
            estmu(:,comp)= normalize*sum(aux,2);
            if (covoption == 0)|(covoption == 2)
                estcov(:,:,comp) = normalize*(aux*y') - estmu(:,comp)*estmu(:,comp)' ...  %%DBUG--WEIGHTS
                    + regularize*eye(dimens);
            else
                estcov(:,:,comp) = normalize*diag(sum(aux.*y,2)) - diag(estmu(:,comp).^2) ;  %%DBUG--WEIGHTS
            end
            if covoption == 2
            ...
            % kill components
            estpp(comp) = max(sum(normindic(comp,:))-nparsover2,0)/sum(W);  %DBUG--WEIGHTS
            ...
            % if killed==1, it means the estcovin the position "comp", we now
        ...
        countf = countf + 1;
        %indicator variables change size simplest to clear and
        %re-initialize
        clear indic
        clear semi_indic
        for i=1:k
        ...
        if k~=1
            ...
            loglike(countf) = sum(W.*log(realmin+sum(indic)./W)); %.*repmat(W,[size(indic,1),1]))));  %%DBUG--WEIGHTS
        else
            % if it is just one component, it is even simpler
            loglike(countf) = sum(W.*log(realmin+indic./W)); %.*repmat(W,[size(indic,1),1])));  %%DBUG--WEIGHTS
        end
    ...
    if dimens == 1
        ...
        set(gca,'FontName','Times','FontSize',22);
        for comp=1:k
            elipsnorm(estmu([axis1,axis2],comp),estcov([axis1,axis2],[axis1,axis2],comp),[.5,1,2,3])
        end
        drawnow
        hold off
    end
...
if dimens == 1
    ...
    for comp=1:length(bestpp)
        elipsnorm(bestmu([axis1,axis2],comp),bestcov([axis1,axis2],[axis1,axis2],comp),[.5,1,2,3])
    end
    drawnow
    hold off
end
